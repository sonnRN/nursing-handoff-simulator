...
if (typeof unique !== 'function') {
  globalThis.unique = function unique(items) {
    return Array.from(new Set((Array.isArray(items) ? items : [items]).filter(Boolean)));
  };
}

if (typeof normalizeNarrativeText !== 'function') {
  globalThis.normalizeNarrativeText = function normalizeNarrativeText(value) {
    return String(value || '').replace(/\s+/g, ' ').trim();
  };
}

if (typeof normalizeClinicalPlaceholderText !== 'function') {
  globalThis.normalizeClinicalPlaceholderText = function normalizeClinicalPlaceholderText(value) {
    const text = normalizeNarrativeText(value);
    if (!text) return '-';
    return /^(none|n\/a|null|undefined|-+)$/i.test(text) ? '-' : text;
  };
}

if (typeof truncateText !== 'function') {
  globalThis.truncateText = function truncateText(value, maxLength = 120) {
    const text = normalizeNarrativeText(value);
