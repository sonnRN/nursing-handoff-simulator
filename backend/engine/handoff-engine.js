(function attachCanonicalHandoffEngine(root) {
  ...
  const RAW_FHIR_SUFFIX_PATTERN = /\s*\((?:disorder|finding|situation|procedure)\)\s*/gi;
  const PLACEHOLDER_PATTERNS = [
    /^(?:-|none|n\/a|null|undefined|unknown)$/i,
    /information\s+missing/i,
    /no\s+(?:diagnosis|data|information)/i,
    /fhir.+missing/i,
    /정보\s*없음/i,
    /자료\s*없음/i,
    /데이터\s*없음/i,
  ...
  const NON_ACTIVITY_PATTERNS = /(counsel|education|teaching|therapy|consult)/i;
  const ACTIVITY_PATTERNS = /(bed rest|ambulation|walker|assist|transfer|chair|turn|reposition|activity|mobility|rest)/i;
  const DEVICE_NOISE_PATTERNS = /\b(ct|mri|x-ray|ultrasound|contrast|stent|consult|request)\b|physical object/i;
  const VENT_NOISE_PATTERNS = /^(?:-|none|room air|ra)$/i;
  ...
  function cleanClinicalText(value) {
    return compactText(
      stripHtml(value)
        .replace(RAW_FHIR_SUFFIX_PATTERN, " ")
  ...
  function cleanList(items) {
    const sourceItems = Array.isArray(items) ? items : (items == null ? [] : [items]);
    return unique(sourceItems.map((item) => cleanClinicalText(item)).filter((item) => !isPlaceholderText(item)));
  }
  ...
  function parseBloodPressure(bp) {
    ...
    return match ? { systolic: Number(match[1]), diastolic: Number(match[2]) } : { systolic: null, diastolic: null };
  }

  function normalizeDiagnosisItems(items) {
    return cleanList(items).filter((item) => !/^fhir$/i.test(item));
  }

  function normalizeActivity(value) {
    const text = cleanClinicalText(value);
    if (isPlaceholderText(text)) return "-";
    if (NON_ACTIVITY_PATTERNS.test(text) && !ACTIVITY_PATTERNS.test(text)) return "-";
    return text || "-";
  }

  function isMeaningfulDeviceItem(kind, value) {
    const text = cleanClinicalText(value);
    if (isPlaceholderText(text)) return false;
    if (DEVICE_NOISE_PATTERNS.test(text)) return false;
    if (kind === "vent" && VENT_NOISE_PATTERNS.test(text)) return false;
    return true;
  }

  function normalizeDeviceItems(kind, items) {
    return cleanList(items).filter((item) => isMeaningfulDeviceItem(kind, item));
  }

  function flattenLabLatest(labs) {
    if (!labs || typeof labs !== "object" || Array.isArray(labs)) return {};
    const entries = [];
    Object.entries(labs).forEach(([key, value]) => {
      ...
      if (typeof value === "object") {
        Object.entries(value).forEach(([nestedKey, nestedValue]) => {
          if (nestedValue == null || typeof nestedValue === "object") return;
  ...
  function detectVitalAbnormalFlags(vital) {
    const bp = parseBloodPressure(vital?.bp);
    const hr = Number(vital?.hr);
    const bt = Number(vital?.bt);
    const rr = Number(vital?.rr);
    const spo2 = Number(vital?.spo2);
    const flags = [];
    if ((bp.systolic != null && bp.systolic < 90) || (bp.diastolic != null && bp.diastolic < 60)) flags.push("bp");
    if (Number.isFinite(hr) && (hr >= 120 || hr <= 50)) flags.push("hr");
    if (Number.isFinite(bt) && (bt >= 38 || bt < 36)) flags.push("bt");
    if (Number.isFinite(rr) && (rr >= 23 || rr <= 10)) flags.push("rr");
    if (Number.isFinite(spo2) && spo2 <= 92) flags.push("spo2");
    return unique(flags);
  }
  ...
  function getLabStatus(key, value) {
    ...
    if (!Number.isFinite(numericValue)) return { status: "unknown", numericValue };
    if (normalizedKey === "k" || normalizedKey.includes("potassium")) {
      if (numericValue > 5.5) return { status: "high", numericValue };
      if (numericValue < 3.5) return { status: "low", numericValue };
    }
    if (normalizedKey === "na" || normalizedKey.includes("sodium")) {
      if (numericValue > 145) return { status: "high", numericValue };
      if (numericValue < 130) return { status: "low", numericValue };
    }
    if (normalizedKey.includes("cr") || normalizedKey.includes("creatinine")) {
      if (numericValue > 1.3) return { status: "high", numericValue };
    }
    if (normalizedKey.includes("bun")) {
      if (numericValue > 20) return { status: "high", numericValue };
    }
    if (normalizedKey.includes("crp")) {
      if (numericValue > 1) return { status: "high", numericValue };
    }
    if (normalizedKey.includes("wbc")) {
      if (numericValue > 12) return { status: "high", numericValue };
      if (numericValue < 4) return { status: "low", numericValue };
    }
    if (normalizedKey === "hb" || normalizedKey === "hgb") {
      if (numericValue < 10) return { status: "low", numericValue };
    }
    if (normalizedKey.includes("plt") || normalizedKey.includes("platelet")) {
      if (numericValue < 150) return { status: "low", numericValue };
    }
    return { status: "normal", numericValue };
  }

  function buildAbnormalLabList(latestLabs) {
    return Object.entries(latestLabs || {})
      .map(([key, value]) => {
        const status = getLabStatus(key, value);
        if (status.status === "normal" || status.status === "unknown") return null;
        return {
          key,
          value: String(value),
  ...
  function normalizeOrders(metaOrders, dayOrders) {
    const source = metaOrders && typeof metaOrders === "object" ? metaOrders : {};
    return {
      active: cleanList(source.active || dayOrders?.active || []),
      routine: cleanList(source.routine || dayOrders?.routine || []),
      prn: cleanList(source.prn || dayOrders?.prn || []),
      medications: {
        inj: cleanList(source.medications?.inj || dayOrders?.medications?.inj || []),
        po: cleanList(source.medications?.po || dayOrders?.medications?.po || []),
        running: cleanList(source.medications?.running || dayOrders?.medications?.running || [])
      }
    };
  }

  function normalizeNursingProblem(value, diagnoses) {
    const lines = String(value || "")
      .split(/\n+/)
      .map((line) => cleanClinicalText(line))
      .map((line) => line
        .replace(/^[-#\s]*/, "")
        .replace(/^\d+\.\s*/, "")
        .replace(/^(?:major issue|nursing focus|nursing problem|problem list)\s*:\s*/i, "")
        .trim())
      .filter(Boolean);
    const filtered = lines.filter((line) => {
      if (isPlaceholderText(line)) return false;
      if (DEVICE_NOISE_PATTERNS.test(line)) return false;
      if (NON_ACTIVITY_PATTERNS.test(line)) return false;
      return true;
    });
    if (filtered.length) return filtered[0];
    const diagnosisText = normalizeDiagnosisItems(diagnoses);
    return diagnosisText[0] || "-";
  }

  function normalizeCarryoverItems(actions) {
    return cleanList(actions).filter((item) => {
      if (DEVICE_NOISE_PATTERNS.test(item)) return false;
      if (/\b(progress|follow up sepsis|observation)\b/i.test(item)) return false;
      if (/경과\s*관찰/i.test(item) && !/(lab|vital|pump|line|tube|drain|report|notify|safety|gait|dressing|식이|드레싱)/i.test(item)) {
        return false;
      }
      return true;
    });
  ...
  function buildVitalsSummary(vital) {
    return compactText(
      `BP ${vital?.bp || "-"}, HR ${vital?.hr ?? "-"}, BT ${vital?.bt ?? "-"}, RR ${vital?.rr ?? "-"}, SpO2 ${vital?.spo2 ?? "-"}%`
    );
  }

  function buildLabsSummary(labs) {
    return Object.entries(labs || {})
      .map(([key, value]) => `${key} ${value}`)
      .join(", ");
  }

  function fallbackBuildNormalizedDailyTimeline(patient, dates) {
    const timelineDates = Array.isArray(dates) && dates.length ? dates.slice() : Object.keys(patient?.dailyData || {}).sort();
    return timelineDates
      .filter((date) => patient?.dailyData?.[date])
      .map((date) => {
        const day = patient.dailyData[date] || {};
        const meta = day.handoffMeta || {};
        const orders = normalizeOrders(meta.orders, day.orders);
        const latestVitals = {
          ...(day.vital || {}),
          ...(meta.vitals?.latest || {})
        };
        const bp = parseBloodPressure(latestVitals.bp);
        const abnormalFlags = unique(meta.vitals?.abnormalFlags || detectVitalAbnormalFlags(latestVitals));
        const latestLabs = {
          ...flattenLabLatest(day.labs),
          ...(meta.labs?.latest || {})
        };
        const abnormalLabs = Array.isArray(meta.labs?.abnormal) && meta.labs.abnormal.length
          ? meta.labs.abnormal.map((item) => ({
              key: cleanClinicalText(item.key),
              value: String(item.value),
              status: item.status || getLabStatus(item.key, item.value).status
            })).filter((item) => item.key && item.status !== "normal")
          : buildAbnormalLabList(latestLabs);
        const diagnoses = normalizeDiagnosisItems(meta.clinicalStatus?.diagnoses || [patient?.diagnosis]);
        const nursingProblem = normalizeNursingProblem(day.nursingProblem, diagnoses);
        const clinicalStatus = {
          diagnoses,
          isolation: isPlaceholderText(meta.clinicalStatus?.isolation) ? "-" : cleanClinicalText(meta.clinicalStatus?.isolation),
          activity: normalizeActivity(meta.clinicalStatus?.activity || day.activity),
          caution: cleanList(meta.clinicalStatus?.caution || []),
          lines: normalizeDeviceItems("lines", meta.clinicalStatus?.lines || []),
          tubes: normalizeDeviceItems("tubes", meta.clinicalStatus?.tubes || []),
          drains: normalizeDeviceItems("drains", meta.clinicalStatus?.drains || []),
          vent: normalizeDeviceItems("vent", meta.clinicalStatus?.vent || [])
        };
        clinicalStatus.activeDevices = unique([
          ...clinicalStatus.lines,
          ...clinicalStatus.tubes,
          ...clinicalStatus.drains,
          ...clinicalStatus.vent
        ]);
        const nursingActions = {
          completed: cleanList(meta.nursingActions?.completed || []),
          pending: cleanList(meta.nursingActions?.pending || []),
          followUp: cleanList(meta.nursingActions?.followUp || [])
        };
        return {
          date,
          nursingProblem,
          patientContext: {
            diagnosis: diagnoses.join(", ") || cleanClinicalText(patient?.diagnosis) || "-",
            admissionReason: cleanClinicalText(patient?.admissionNote || patient?.admitReason),
            pastHistory: cleanList(patient?.pastHistory || day.pastHistory || []),
            diet: isPlaceholderText(patient?.diet || day?.diet) ? "-" : cleanClinicalText(patient?.diet || day?.diet)
          },
          clinicalStatus,
          orders,
          vitals: {
            ...
            abnormalFlags,
            summaryText: buildVitalsSummary(latestVitals)
          },
          labs: {
            latest: latestLabs,
            abnormal: abnormalLabs,
            summaryText: buildLabsSummary(latestLabs)
          },
          nursingActions,
          carryover: {
            items: normalizeCarryoverItems([
              ...nursingActions.pending,
              ...nursingActions.followUp
            ])
          },
          specials: cleanList(day.specials || []),
          consults: cleanList(day.consults || []),
          hourly: Array.isArray(day.hourly) ? day.hourly : [],
          sourceRefs: meta.sourceRefs || {}
        };
      });
  }

  function buildIdentityBasis(patient, current, timeline) {
    const basis = [];
    if (current?.patientContext?.admissionReason && current.patientContext.admissionReason !== "-") {
      basis.push(`Admission note: ${current.patientContext.admissionReason}`);
    }
    if ((current?.specials || []).length) {
      basis.push(`${current.date} imaging/special: ${current.specials.slice(0, 2).join(", ")}`);
    }
    if ((current?.patientContext?.pastHistory || []).length) {
      basis.push(`Past history: ${current.patientContext.pastHistory.join(", ")}`);
    }
    if (!basis.length && timeline[0]?.date) {
      basis.push(`Observed across ${timeline[0].date} to ${timeline[timeline.length - 1].date}`);
    }
    return basis.slice(0, 4);
  }

  function buildNursingProblemBasis(problem, current) {
    const basis = [];
    if (current?.date) basis.push(`${current.date} nursing focus: ${problem}`);
    if (current?.clinicalStatus?.activity && current.clinicalStatus.activity !== "-") {
      basis.push(`Activity/support level: ${current.clinicalStatus.activity}`);
    }
    const deviceSupport = unique([
      ...(current?.clinicalStatus?.tubes || []),
      ...(current?.clinicalStatus?.lines || []),
      current?.patientContext?.diet
    ]).filter((item) => item && item !== "-");
    if (deviceSupport.length) {
      basis.push(`Device/diet support: ${deviceSupport.slice(0, 3).join(", ")}`);
    }
    if ((current?.clinicalStatus?.caution || []).length) {
      basis.push(`Safety caution: ${current.clinicalStatus.caution.join(", ")}`);
    }
    return basis.slice(0, 4);
  }
  ...
  function createSummaryItem(input) {
    return {
      ...
      summary: cleanClinicalText(input.summary),
      detail: cleanClinicalText(input.detail),
      clinicalBasis: cleanList(input.clinicalBasis || []).slice(0, 4),
      evidence: cleanList(input.evidence || []).slice(0, 4),
      sourceDates: unique(input.sourceDates || []),
      importanceBand: input.importanceBand || "background",
      linkActions: Array.isArray(input.linkActions) ? input.linkActions.slice() : [],
      hideEvidence: Boolean(input.hideEvidence),
      showSourceDates: input.showSourceDates !== false
    };
  }

  function buildIdentitySection(patient, timeline) {
    const current = timeline[timeline.length - 1];
    const diagnosisLabel = (current?.clinicalStatus?.diagnoses || [cleanClinicalText(patient?.diagnosis)]).filter(Boolean)[0];
    const items = [];
    if (diagnosisLabel) {
      items.push(createSummaryItem({
        id: `identity:diagnosis:${diagnosisLabel}`,
        category: "identity",
        summary: `Main diagnosis: ${diagnosisLabel}`,
        detail: "Primary diagnosis driving the current handoff context.",
        clinicalBasis: buildIdentityBasis(patient, current, timeline),
    ...
    if ((current?.patientContext?.pastHistory || []).length) {
      items.push(createSummaryItem({
        id: "identity:history",
        category: "identity",
        summary: `Past history: ${current.patientContext.pastHistory.join(", ")}`,
        detail: "Relevant background conditions for ongoing care.",
        clinicalBasis: [`Past history list: ${current.patientContext.pastHistory.join(", ")}`],
        sourceDates: [timeline[0]?.date].filter(Boolean),
  ...
  function buildCareFrameSection(timeline) {
    const current = timeline[timeline.length - 1];
    const items = [];
    if (current?.clinicalStatus?.activity && current.clinicalStatus.activity !== "-") {
      items.push(createSummaryItem({
        ...
        category: "care_frame",
        summary: `Activity: ${current.clinicalStatus.activity}`,
        detail: "Current mobility and assistance requirement.",
        sourceDates: [current.date],
        importanceBand: "focus",
        hideEvidence: true
      }));
    }
    if (current?.clinicalStatus?.isolation && current.clinicalStatus.isolation !== "-") {
      items.push(createSummaryItem({
        ...
        category: "care_frame",
        summary: `Isolation: ${current.clinicalStatus.isolation}`,
        detail: "Current isolation or precaution status.",
        sourceDates: [current.date],
        importanceBand: "supporting",
        hideEvidence: true
      }));
    ...
    if ((current?.clinicalStatus?.caution || []).length) {
      items.push(createSummaryItem({
        id: "care:caution",
        category: "care_frame",
        summary: `Caution: ${current.clinicalStatus.caution.join(", ")}`,
        detail: "Important precautions for the next shift.",
        sourceDates: [current.date],
        importanceBand: "focus",
        hideEvidence: true
      }));
    ...
    [
      ...
      if ((current?.clinicalStatus?.[key] || []).length) {
        items.push(createSummaryItem({
          id: `care:${key}`,
          category: "care_frame",
          summary: `${label}: ${current.clinicalStatus[key].join(", ")}`,
          detail: `Current ${label.toLowerCase()} to hand off safely.`,
          sourceDates: [current.date],
          importanceBand: key === "vent" ? "focus" : "supporting",
          hideEvidence: true
        }));
      }
    });
  ...
  function buildPersistentConcernSection(timeline) {
    const current = timeline[timeline.length - 1];
    const items = [];
    if (current?.nursingProblem && current.nursingProblem !== "-") {
      const looksLikeSelfCareDeficit =
        /self-care deficit/i.test(current.nursingProblem)
        || (
          (/stroke|infarction/i.test((current?.clinicalStatus?.diagnoses || []).join(" ")))
          && (
            /assist/i.test(current?.clinicalStatus?.activity || "")
            || (current?.clinicalStatus?.tubes || []).some((item) => /l-tube/i.test(item))
            || /l-tube/i.test(current?.patientContext?.diet || "")
          )
        );
      const summary = looksLikeSelfCareDeficit
        ? "NANDA nursing diagnosis: Self-care deficit"
        : `Persistent concern: ${current.nursingProblem}`;
      items.push(createSummaryItem({
        id: `persistent:nursing:${current.nursingProblem}`,
        category: "persistent_problem",
        summary,
        detail: "Problem still active and relevant for the next handoff.",
        clinicalBasis: buildNursingProblemBasis(current.nursingProblem, current),
        sourceDates: timeline.map((snapshot) => snapshot.date),
        importanceBand: "core"
      }));
    }
    const persistentFlags = unique(timeline.flatMap((snapshot) => snapshot?.vitals?.abnormalFlags || []))
      .filter((flag) => (current?.vitals?.abnormalFlags || []).includes(flag));
    if (persistentFlags.length) {
      items.push(createSummaryItem({
        id: `persistent:vitals:${persistentFlags.join("|")}`,
        category: "persistent_problem",
        summary: `Persistent vital concern: ${persistentFlags.join(", ")}`,
        detail: "Abnormal vital trend still active in the latest snapshot.",
        clinicalBasis: [`${current.date} vital signs: ${current.vitals.summaryText}`],
        sourceDates: timeline.filter((snapshot) => (snapshot?.vitals?.abnormalFlags || []).some((flag) => persistentFlags.includes(flag))).map((snapshot) => snapshot.date),
        importanceBand: "focus",
        linkActions: [{ kind: "vital", label: "V/S 보기", date: current.date }]
      }));
    }
    return items;
  }

  function buildWatchSection(timeline) {
    const current = timeline[timeline.length - 1];
    const items = [];
    if ((current?.vitals?.abnormalFlags || []).length) {
      items.push(createSummaryItem({
        id: `watch:vitals:${current.vitals.abnormalFlags.join("|")}`,
        category: "watch_item",
        summary: `Watch vitals: ${current.vitals.abnormalFlags.join(", ")}`,
        detail: "Latest abnormal vital signs need immediate awareness.",
        clinicalBasis: [`${current.date} vital signs: ${current.vitals.summaryText}`],
        sourceDates: [current.date],
        importanceBand: "focus",
        linkActions: [{ kind: "vital", label: "V/S 보기", date: current.date }]
    ...
    if ((current?.labs?.abnormal || []).length) {
      const label = current.labs.abnormal.map((item) => `${item.key} ${item.value}`).join(", ");
      items.push(createSummaryItem({
        id: `watch:labs:${current.labs.abnormal.map((item) => item.key).join("|")}`,
        category: "watch_item",
        summary: `Watch labs: ${label}`,
        detail: "Latest abnormal labs need follow-up or reporting.",
        clinicalBasis: [`${current.date} labs: ${label}`],
        sourceDates: [current.date],
        importanceBand: "focus",
        linkActions: [{ kind: "lab", label: "Lab 보기", labKeys: current.labs.abnormal.map((item) => item.key) }]
      }));
    }
    ...
    if (highRiskOrders.length) {
      items.push(createSummaryItem({
        id: "watch:high-risk-order",
        category: "watch_item",
        summary: `High-risk medication/order: ${highRiskOrders.join(", ")}`,
        detail: "Medication or infusion requiring close next-shift attention.",
        clinicalBasis: [`${current.date} active high-risk order: ${highRiskOrders.join(", ")}`],
        sourceDates: [current.date],
        importanceBand: "focus"
      }));
    }
  ...
  function buildCarryoverSection(timeline) {
    const current = timeline[timeline.length - 1];
    return (current?.carryover?.items || []).slice(0, 6).map((item) => createSummaryItem({
      id: `carryover:${item}`,
      category: "carryover",
      summary: `Carryover: ${item}`,
      detail: "Pending follow-up that the next shift should continue.",
      clinicalBasis: [`${current.date} pending action: ${item}`],
      sourceDates: [current.date],
      importanceBand: "focus",
      linkActions: /(vital|bp|hr|rr|spo2|v\/s)/i.test(item)
        ? [{ kind: "vital", label: "V/S 보기", date: current.date }]
        : (/\blab\b|wbc|crp|na|k|bun|cr|hb/i.test(item)
          ? [{ kind: "lab", label: "Lab 보기", labKeys: (current?.labs?.abnormal || []).map((lab) => lab.key).slice(0, 4) }]
          : [])
    }));
  }
  ...
  function fallbackBuildLongitudinalPatientSummary(patient, timeline) {
    if (!Array.isArray(timeline) || !timeline.length) {
      return {
        ...
        conciseLines: [],
        conciseSummary: "Summary information is unavailable.",
        debug: { candidateCount: 0 }
      };
    }

    const sections = {
      identity: buildIdentitySection(patient, timeline),
      careFrame: buildCareFrameSection(timeline),
      persistentConcerns: buildPersistentConcernSection(timeline),
      watchItems: buildWatchSection(timeline),
      carryoverItems: buildCarryoverSection(timeline)
    };
    const overview = buildOverview(sections);
    const conciseLines = [];
    if (overview.identity[0]) conciseLines.push(overview.identity[0]);
    if (overview.careFrame.length) conciseLines.push(`Care frame: ${overview.careFrame.slice(0, 2).join(" / ")}`);
    if (overview.persistentConcerns.length) conciseLines.push(`Persistent concerns: ${overview.persistentConcerns.slice(0, 2).join(" / ")}`);
    if (overview.watchItems.length) conciseLines.push(`Watch: ${overview.watchItems.slice(0, 2).join(" / ")}`);
    if (overview.carryoverItems.length) conciseLines.push(`Carryover: ${overview.carryoverItems.slice(0, 2).join(" / ")}`);
    ...
    return {
      ...
      overview,
      conciseLines,
      conciseSummary: conciseLines.join(" | ") || `${cleanClinicalText(patient?.name) || "Patient"} summary unavailable.`,
      dateRange: {
      ...
      debug: {
        candidateCount: Object.values(sections).reduce((sum, items) => sum + items.length, 0)
      }
    };
  }

  function safeOrderList(snapshot) {
    return unique([
      ...(snapshot?.orders?.active || []),
      ...(snapshot?.orders?.routine || []),
      ...(snapshot?.orders?.prn || []),
  ...
  function differenceList(currentItems, previousItems) {
    const previousSet = new Set((previousItems || []).map((item) => cleanClinicalText(item)));
    return (currentItems || []).filter((item) => !previousSet.has(cleanClinicalText(item)));
  }

  function createFallbackEvent(type, date, summary, detail, evidence, sbarSection, extra = {}) {
    return {
      id: compactText(`${type}:${date}:${summary}`),
      type,
      date,
      summary: cleanClinicalText(summary),
      detail: cleanClinicalText(detail),
  ...
  function fallbackDetectHandoffChanges(previous, current) {
    if (!current) return [];
    const events = [];
    const currentVitalFlags = current.vitals?.abnormalFlags || [];
    if (currentVitalFlags.length) {
      events.push(createFallbackEvent(
        "vital_abnormal",
        current.date,
        `Vital change: ${current.vitals.summaryText}`,
        "Latest abnormal vital signs need prompt review.",
        [`${current.date} ${current.vitals.summaryText}`],
        "situation",
        { priorityBand: currentVitalFlags.some((flag) => flag === "bp" || flag === "spo2") ? "urgent" : "high" }
      ));
    }
    if ((current.labs?.abnormal || []).length) {
      const label = current.labs.abnormal.map((item) => `${item.key} ${item.value}`).join(", ");
      events.push(createFallbackEvent(
        ...
        current.date,
        `Abnormal labs: ${label}`,
        "Latest abnormal lab values need follow-up.",
        [`${current.date} labs ${label}`],
        "assessment",
        { priorityBand: "high" }
      ));
    }

    differenceList(safeOrderList(current), safeOrderList(previous)).forEach((item) => {
      events.push(createFallbackEvent(
        "new_order",
        current.date,
        `New order: ${item}`,
        "New medication or treatment added since the previous snapshot.",
        [`${current.date} order ${item}`],
        "situation",
        { priorityBand: detectHighRiskMedication(item) ? "urgent" : "high" }
      ));
    });
    ...
    [
      ...
      const currentValue = Array.isArray(current?.clinicalStatus?.[key])
        ? current.clinicalStatus[key].join(", ")
        : current?.clinicalStatus?.[key] || "-";
      const previousValue = Array.isArray(previous?.clinicalStatus?.[key])
        ? previous.clinicalStatus[key].join(", ")
        : previous?.clinicalStatus?.[key] || "-";
      if (currentValue !== previousValue && currentValue !== "-") {
        events.push(createFallbackEvent(
          "status_change",
          current.date,
          `${label} changed: ${currentValue}`,
          `Previous: ${previousValue || "-"}`,
          [`${current.date} ${label} ${currentValue}`],
          "background",
          { priorityBand: key === "vent" ? "high" : "moderate" }
        ));
      }
    });

    (current.carryover?.items || []).forEach((item) => {
      events.push(createFallbackEvent(
        "nursing_action",
        current.date,
        `Pending follow-up: ${item}`,
        "Carryover item still requires next-shift follow-up.",
        [`${current.date} pending ${item}`],
        "recommendation",
  ...
  function fallbackBuildSbarPayload(patient, context) {
    const items = Array.isArray(context?.prioritizedHandoffItems) && context.prioritizedHandoffItems.length
      ? context.prioritizedHandoffItems
      : Array.isArray(context?.timelineEvents) ? context.timelineEvents : [];
    const current = context?.current || null;
    const diagnosis = current?.clinicalStatus?.diagnoses?.join(", ") || cleanClinicalText(patient?.diagnosis) || "-";
    const background = [
      createFallbackEvent(
        "background",
        current?.date || "",
        `Diagnosis context: ${diagnosis}`,
        current?.patientContext?.admissionReason || "Admission context carried into the current handoff.",
        [diagnosis],
        "background",
        { priorityBand: "moderate" }
      )
    ...
    if (current?.clinicalStatus?.activity && current.clinicalStatus.activity !== "-") {
      background.push(createFallbackEvent(
        "background",
        current.date,
        `Current activity: ${current.clinicalStatus.activity}`,
        "Mobility and assistance status for the next shift.",
        [`${current.date} activity ${current.clinicalStatus.activity}`],
        "background",
        { priorityBand: "moderate" }
      ));
    }
    ...
    return {
      situation: items.filter((item) => item.sbarSection === "situation" || item.type === "vital_abnormal" || item.type === "new_order").slice(0, 4),
      background: background.slice(0, 4),
      assessment: items.filter((item) => item.sbarSection === "assessment" || item.type === "lab_change" || item.type === "status_change").slice(0, 4),
      recommendation: items.filter((item) => item.sbarSection === "recommendation" || item.type === "nursing_action").slice(0, 4)
    };
  }
  ...
  function canonicalBuildNormalizedDailyTimeline(patient, dates) {
    const timeline = legacy.buildNormalizedDailyTimeline
      ...
      : fallbackBuildNormalizedDailyTimeline(patient, dates);
  ...
  function canonicalBuildLongitudinalPatientSummary(patient, timeline, policy) {
    const summary = legacy.buildLongitudinalPatientSummary
      ...
      : fallbackBuildLongitudinalPatientSummary(patient, timeline, policy);
  ...
  function buildCanonicalSbarPayload(patient, legacyAnalysis, prioritizedItems, current, previous, departmentProfile) {
    ...
    const basePayload = legacyAnalysis?.sbarPayload || (legacy.buildSbarPayload
      ...
      : fallbackBuildSbarPayload(patient, {
          current,
          previous,
          prioritizedHandoffItems: prioritizedItems,
          timelineEvents
  ...
  function fallbackScoreHandoffEvents(events) {
  ...
  function canonicalBuildHandoffAnalysis(patient, dates, policy) {
    ...
    const rawChangeEvents = Array.isArray(legacyAnalysis?.changeEvents)
      ...
      : (legacy.detectHandoffChanges && current
        ...
        : fallbackDetectHandoffChanges(previous, current, normalizedDailyTimeline[0] || null));
  ...
  root.__canonicalBuildHandoffAnalysis = canonicalBuildHandoffAnalysis;
  root.__canonicalGenerateNarrativeSBAR = canonicalGenerateNarrativeSBAR;
  root.detectHandoffChanges = root.detectHandoffChanges || fallbackDetectHandoffChanges;
  root.scoreHandoffEvents = root.scoreHandoffEvents || fallbackScoreHandoffEvents;
  root.buildSbarPayload = root.buildSbarPayload || fallbackBuildSbarPayload;
